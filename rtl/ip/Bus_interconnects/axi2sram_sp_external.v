        ...
        case (state_q)
            ...
            WAIT_WVALID: begin
                ...
                if (s_wvalid) begin
                    req_o          = 1'b1;
                    we_o           = 1'b1;
                    // Mirror IDLE's first-beat transition: insert a WRITE_NOP
                    // between this beat and the next. The external SRAM model
                    // (sim/sram.v) latches on the rising edge of a combinational
                    // write strobe; without the NOP the first two beats share one
                    // strobe pulse and the second beat's write is lost.
                    state_d        = (s_wlast) ? SEND_B : WRITE_NOP;
                    cnt_d          = 1;
                end
            end
